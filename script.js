...
function logout() {
  ...
  // Mostrar la landing page completa
...
document.body.style.overflow = '';
  ...
  if (screen === 'login' || screen === 'register' || usuarioActual) {
    // Ocultar landing page cuando se muestra login/register o cuando hay usuario logueado
    landingElements.forEach(el => el.style.display = 'none');
    // Mostrar menú inferior si hay usuario logueado
    document.getElementById('menuInferior').style.display = usuarioActual ? 'flex' : 'none';
  } else {
  ...
  // Renderiza el contenido específico
  switch (screen) {
...
function renderDashboard() {
  ...
  productosOrdenados.forEach((producto, index) => {
    ...
    const textoDias = dias > 0 ? `${dias} días restantes` : 'Expirado';
      ...
      <div class="bg-white shadow rounded-xl overflow-hidden flex flex-col">
        ...
        <div class="p-4 flex-1 flex flex-col justify-between">
          ...
          <div class="flex justify-between text-sm text-gray-600">
            ...
            <span class="flex items-center gap-1">
              <i class="fa-solid fa-utensils"></i> desayuno
            </span>
          </div>
        </div>
      </div>
      <!-- Segunda tarjeta de receta -->
      <div class="bg-white shadow rounded-xl overflow-hidden flex flex-col">
        <div class="relative">
          <img 
...
window.onload = () => {
  ...
  `;
  // Asegúrate de que todas las pantallas estén ocultas al inicio
  document.querySelectorAll('.screen').forEach(s => s.classList.add('hidden'));
  
  // Muestra la landing page (header, secciones y footer)
  document.querySelectorAll('header, section, footer').forEach(el => {
    el.style.display = 'block';
  });
